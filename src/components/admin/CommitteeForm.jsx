...
const CommitteeForm = ({ committee = null, onSubmit }) => {
  const [formData, setFormData] = useState({
    name: committee?.name || '',
    description: committee?.description || '',
    type: committee?.type || 'administrative', // administrative, scientific
  ...
  const committeeTypes = [
    ...
    { value: 'scientific', label: 'علمية واستشارية' },
  ];
  ...
  const validateForm = () => {
    ...
    if (!formData.description.trim()) {
      newErrors.description = 'وصف اللجنة مطلوب';
    }
    
    if (!formData.type) {
      newErrors.type = 'نوع اللجنة مطلوب';
    }
    
    if (!formData.head.trim()) {
      newErrors.head = 'اسم رئيس اللجنة مطلوب';
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="border-t border-gray-200 pt-4">
        <h3 className="text-lg font-medium mb-3">أعضاء اللجنة</h3>
      ...
      <div className="flex justify-end">
        ...
        >
          {isSubmitting ? 'جاري الحفظ...' : committee ? 'تحديث اللجنة' : 'إنشاء اللجنة'}
        </button>
      </div>
    </form>
  );
